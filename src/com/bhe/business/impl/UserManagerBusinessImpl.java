...
import com.bhe.util.common.SendSmsUtils;
import com.bhe.util.common.Sms;
						...
						if(!CommonUtil.isEmpty(resultMap.get("data"))){
							...
							if ("1".equals(resultStatus)) {
								...
								MailUtil.send(SendMail.SMTP, SendMail.FROM, email, SendMail.TITLE,
										"<h1>百居易电子商务有限公司,</h1><font>您的账号已审核成功，请点击登录链接。</font><a href='http://http://localhost:8080/bheticketview/'>登录</a>",
										SendMail.USERNAME, SendMail.PASSWORD);
							} else if ("2".equals(resultStatus)) {
								// 发送审核失败邮件
								MailUtil.send(SendMail.SMTP, SendMail.FROM, email, SendMail.TITLE,
										"<h1>百居易电子商务有限公司,</h1><font>对不起您的账号审核失败了，请点击链接重新注册。</font><a href='http://http://localhost:8080/bheticketview/toRegist'>注册</a>",
										SendMail.USERNAME, SendMail.PASSWORD);
							}
						}
					}
