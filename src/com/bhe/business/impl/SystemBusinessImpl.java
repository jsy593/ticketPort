							...
							if("1".equals(status)){
								...
								if(!CommonUtil.isEmpty(email)){
									...
									MailUtil.send(SendMail.SMTP,SendMail.FROM,email,SendMail.TITLE,
										"<h1>百居易电子商务有限公司,</h1><font>您申请接入的系统已审核成功，您的appId:"+appId+",appKey:"+appKey+"。请点击链接登录。</font><a href='http://http://localhost:8080/bheticketview/'>登录</a>",SendMail.USERNAME,SendMail.PASSWORD);
								}
								...
								if(!CommonUtil.isEmpty(email)){
									//发送审核失败邮件
									MailUtil.send(SendMail.SMTP,SendMail.FROM,email,SendMail.TITLE,
										"<h1>百居易电子商务有限公司,</h1><font>您申请接入的系统审核失败，请点击登录链接重新申请。</font><a href='http://http://localhost:8080/bheticketview/'>登录</a>",SendMail.USERNAME,SendMail.PASSWORD);
								}
							}
						}
					} catch (Exception e) {
